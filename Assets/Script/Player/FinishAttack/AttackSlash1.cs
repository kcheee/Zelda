...
public class AttackSlash1 : MonoBehaviour
{
    // Start is called before the first frame update
    public GameObject link;
    private void OnEnable()
    {
        Collider[] cols = Physics.OverlapSphere(link.transform.position, 30);
        ...
        for (int i = 0; i < cols.Length; i++)
        {
            ...
            if (cols[i].CompareTag("Bokoblin"))
            {
                ...
                foreach (Rigidbody rb in rigid)
                {
                    ...
                    rb.AddExplosionForce(80 * rb.mass, link.transform.position, 30, 15 * rb.mass, ForceMode.Impulse);
                }
