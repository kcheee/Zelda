...
public class Unity_B : MonoBehaviour
{
    static public Unity_B instance = null;
    private void Awake()
    {
        instance = this;
    }
    ...
    {
        // Debug.Log(state);
        if (state == UnityState.Idle)
        {
            UpdateIdle();
        }
        else if (state == UnityState.Move)
        {
            UpdateMove();
        }
        else if (state == UnityState.Attack)
        {
            UpdateAttack();
        }
        else if (state == UnityState.Damaged)
        {
            UpdateDamaged();
        }
        else if (state == UnityState.Die)
        {
            UpdateDie();
        }
        Debug.Log(state);
    ...
    private void UpdateMove()
    {
        ...
        else if (distance > attackDistance)
        {
            ...
            // transform.position = Vector3.Lerp(transform.position, link.transform.position, 0.1f);
